...
current_frame = 1
b = np.zeros([179, 2])
while current_frame <= 179:
    fig, ax = plt.subplots()
    ax.set_xlim(-50, 180)
    ax.set_ylim(-130, 10)
    ax.set_axis_off()
    draw_0(ax)
    a = draw_1([yao, 0], float(data1[current_frame - 1]), ax)
    b[current_frame - 1] = draw_2(a, float(data2[current_frame - 1])-float(data1[current_frame - 1]), ax)
